...
{
  // std::cout << "me=" << comm->me << "  reached FixTopo::setup_ore_force\n" << std::flush;
...
{
   // std::cout << "me=" << comm->me << "  start FixTopo::pre_exchange\n" << std::flush;
  // update ghosts
  comm->forward_comm();
...
{
  for (int m = 0; m < 3; m++) buf[m] = array[i][m];
  buf[3]=flip[i];
  return 4;
}
...
{
  for (int m = 0; m < 3; m++) array[nlocal][m] = buf[m];
  flip[nlocal]=buf[3];
  return 4;
}
/* ----------------------------------------------------------------------
...
double FixTopo::compute_vector(int n)
{
  ...
  return (double)all;
   // std::cout << "me=" << comm->me << "  end FixTopo::compute_Vector\n" << std::flush;
}
