  ...
  if (modify->nfix && modify->get_fix_by_id("topo")!=nullptr)
    {
       ...
       if (comm->me==0) 
            utils::logmesg(lmp,"Reusing old: fix topo all topo\n");
    ...
    {   
        ...
        if (comm->me==0) 
            utils::logmesg(lmp,"Creating new: fix topo all topo\n");
    }
...
{
  if (comm->me==0) 
            utils::logmesg(lmp,"Destroying pair & fix topo\n");
  ...
  if (modify->nfix && modify->get_fix_by_id(f_topo->id)!=nullptr) modify->delete_fix(f_topo->id);
}
        ...
        if (dx*dxlast+dy*dylast+dz*dzlast < 0)
            {
               ...
               if (debug)
                 {
                       bool i0ghost = i0<atom->nlocal;
                       bool j0ghost = j0<atom->nlocal;
                       bool i1ghost = i1<atom->nlocal;
                       bool j1ghost = j1<atom->nlocal;
                                  
                       std::cout << update->ntimestep << ":   Bond violation " << tag[i0] << "-" << tag[j0] << "  to  " << tag[i1] << "-" << tag[j1] << "  ghosts: " << i0ghost << j0ghost << i1ghost << j1ghost << "\n";       
                 }
